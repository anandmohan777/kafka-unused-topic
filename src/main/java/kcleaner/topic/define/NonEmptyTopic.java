...
public class NonEmptyTopic implements ReservedTopic {
    ...
    private void setNonEmptyTopics() throws ExecutionException, InterruptedException {
        ...
        for (Map.Entry<Integer, Map<String, DescribeLogDirsResponse.LogDirInfo>> nodes : logDirsResult.all().get().entrySet()) {
            for (Map.Entry<String, DescribeLogDirsResponse.LogDirInfo> node : nodes.getValue().entrySet()) {
                for (Map.Entry<TopicPartition, DescribeLogDirsResponse.ReplicaInfo> log : node.getValue().replicaInfos.entrySet()) {
                    if (log.getValue().size > 0) {
                        if(log.getKey().topic().matches(".*" + "_v\\d+" + "$")) {
                            System.out.println(log.getKey().topic() + " ,key: " + node.getKey() + " ,partition: " + log.getKey().partition() + " ,size: " + log.getValue().size / (1024 * 1024) + "Mb");
                        }
